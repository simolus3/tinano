...
class _MyHomePageState extends State<MyHomePage> {
  ...
  StreamController<List<TodoEntry>> todoEntries = new StreamController.broadcast();
  ...
  void _addTodoFromTextField() {
    ...
    if (userTodoText.isNotEmpty) {
      ...
      _database.createTodoEntryAndReturnId(userTodoText)
        .whenComplete(_updateTodosFromDb);
    }
  }

  void _deleteTodoEntry(TodoEntry delete) {
    _database.deleteTodoEntry(delete.id)
      .whenComplete(_updateTodosFromDb);
  }
    ...
    return new Scaffold(
      ...
      body: StreamBuilder(
        ...
        builder: (ctx, AsyncSnapshot<List<TodoEntry>> snapshot) {
          if (snapshot.hasData) {
            final entries = snapshot.data.map(
              (entry) => TodoListWidget(entry, _deleteTodoEntry)
              );
          ...
          child: Row(
            ...
            children: [
              ...
              FloatingActionButton(
                onPressed: _addTodoFromTextField,
...
class TodoListWidget extends StatelessWidget {
  ...
  TodoListWidget(this._entry, this._onDeleted) : super(key: ObjectKey(_entry.id));
